...
use magnus::{Error, RArray, RHash, Symbol, TypedData, Value};
...
impl YXmlElement {
    ...
    pub(crate) fn yxml_element_get(&self, index: u32) -> Option<Value> {
        self.0.borrow()
            ...
            .map(|node| {
                match node {
                    Xml::Element(el) => {
                        Value::from(YXmlElement(RefCell::from(el)))
                    }
                    Xml::Text(text) => {
                        Value::from(YXmlText(RefCell::from(text)))
                    }
                }
            })
    }
    ...
    pub(crate) fn yxml_element_insert_attribute(&self, transaction: &YTransaction, name: String, value: String) {
        self.0.borrow_mut()
            .insert_attribute(&mut *transaction.0.borrow_mut(), name, value);
    }
    pub(crate) fn yxml_element_insert_element(&self, transaction: &YTransaction, index: u32, name: String) {
        self.0.borrow_mut()
            .insert_elem(&mut *transaction.0.borrow_mut(), index, name);
    }
    pub(crate) fn yxml_element_insert_text(&self, transaction: &YTransaction, index: u32) {
        self.0.borrow_mut()
    ...
    pub(crate) fn yxml_element_observe(&self, block: Proc) -> Result<u32, Error> {
        ...
        let subscription_id = self.0.borrow_mut()
            .observe(move |transaction, xml_element_event| {
                ...
                for change in delta {
                    match change {
                        Change::Added(v) => {
                            ...
                            payload.aset(change_added, RArray::from_vec(values)).expect("cannot create change::added payload");

                            changes.push(payload).expect("cannot push payload to list of changes");
                        }
                        Change::Retain(position) => {
                            let mut payload = RHash::new();
                            payload.aset(change_retain, *position).expect("cannot create change::retain payload");

                            changes.push(payload).expect("cannot push payload to list of changes");
                        }
                        Change::Removed(position) => {
                            let mut payload = RHash::new();
                            payload.aset(change_removed, *position).expect("cannot create change::removed payload");
                ...
                block.call::<(RArray, ), Value>((changes, )).expect("cannot call block");
            });
    ...
    pub(crate) fn yxml_element_push_element_back(&self, transaction: &YTransaction, name: String) -> YXmlElement {
        let xml_element = self.0.borrow_mut()
            .push_elem_back(&mut *transaction.0.borrow_mut(), name);

        YXmlElement(RefCell::from(xml_element))
    }
    pub(crate) fn yxml_element_push_element_front(&self, transaction: &YTransaction, name: String) -> YXmlElement {
        let xml_element = self.0.borrow_mut()
    ...
    pub(crate) fn yxml_element_push_text_back(&self, transaction: &YTransaction) -> YXmlText {
        let xml_text = self.0.borrow_mut()
    ...
    pub(crate) fn yxml_element_remove_range(&self, transaction: &YTransaction, index: u32, length: u32) {
        self.0.borrow_mut()
